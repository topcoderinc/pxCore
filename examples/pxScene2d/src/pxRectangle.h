...
public:
  ...
  rtProperty(radius, radius, setRadius, float);

  pxRectangle(pxScene2d* scene):pxObject(scene),mLineWidth(0), mRadius(0)
  {
    float f[4] = {0,0,0,1};
    float l[4] = {1,1,1,1};
    setFillColor(f);
  ...
  rtError radius(float &r) const {
    r = mRadius;
    return RT_OK;
  }

  rtError setRadius(float r) {
    mRadius = r;
    return RT_OK;
  }
